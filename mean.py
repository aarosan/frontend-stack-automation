...
def create_client(destination):
    # Extract the project name from the destination directory
    project_name = "client"

    try:
        # Run Angular CLI to create the Angular project with default options
        subprocess.run([
            "npx", "@angular/cli", "new", project_name, "--defaults", "--skip-git"
        ], cwd=destination, check=True)
        print(f"Created Angular app successfully in '{os.path.join(destination, project_name)}'.")
        
    except subprocess.CalledProcessError as e:
        print(f"An error occurred while creating the Angular app: {e}")
        return
    
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python3 mean.py [project_name] [source_directory]")
        sys.exit(1)
