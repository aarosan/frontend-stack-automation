...
def initialize_root_package_json(destination):
    try:
        ...
        package_json["scripts"]["start"] = "concurrently \"cd server && npm start\" \"cd client && npm run serve\""
...
def create_client(destination):
    ...
    client_directory = os.path.join(destination, "client")

    # Ensure the client directory exists
    os.makedirs(client_directory, exist_ok=True)
    
    # Run Vue CLI to create the Vue project inside the client directory
    # Will need to confirm with Vue to run inside current directory
    try:
        subprocess.run([
            "npx", "@vue/cli", "create", ".", "--default", "--no-git"
        ], cwd=client_directory, check=True)

        print(f"Created Vue app successfully in '{client_directory}'.")
    except subprocess.CalledProcessError as e:
        print(f"An error occurred while creating the Vue app: {e}")
        return

if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python3 mevn.py [project_name] [source_directory]")
        sys.exit(1)
